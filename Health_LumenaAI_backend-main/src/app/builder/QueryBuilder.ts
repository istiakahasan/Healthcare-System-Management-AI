...
class QueryBuilder {
  ...
  search(searchableFields: string[]) {
    const searchTerm = this.query.searchTerm as string;
    if (searchTerm) {
      const orConditions = searchableFields.map((fieldPath) => {
        const keys = fieldPath.split("."); // ["staff","user","firstName"]
        let condition: any = { [keys.pop()!]: { contains: searchTerm, mode: "insensitive" } };
